...
instruction *file_reader::get_next_instruction() {
    ...
    try {
        if (regex_search(buffer, matches, regex(INSTRUCTION_WITH_COMMENT))) {
        ...
        } else if (regex_search(buffer, matches, regex(INSTRUCTION_WITHOUT_COMMENT))) {
            instruction->set_label(matches[1].str());
            instruction->set_mnemonic(new mnemonic(matches[2].str()));
            instruction->set_operand(new operand(matches[3].str()));
        } else if (regex_search(buffer, matches, regex(INSTRUCTION_WITHOUT_OPERAND_WITH_COMMENT))) {
            instruction->set_label(matches[1].str());
            instruction->set_mnemonic(new mnemonic(matches[2].str()));
            instruction->set_comment(matches[3]);
        } else if (regex_search(buffer, matches, regex(INSTRUCTION_WITHOUT_OPERAND_WITHOUT_COMMENT))) {
            instruction->set_label(matches[1].str());
            instruction->set_mnemonic(new mnemonic(matches[2].str()));
        } else {
