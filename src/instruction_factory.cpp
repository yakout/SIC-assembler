...
instruction* instruction_factory::make_instruction(std::string _location, std::string _label, mnemonic* _mnemonic,
    ...
    if (_buffer[8] == '*') {
        // the * indicates that this is literal statemnet in intermediate file.
        _instruction = new literal(_buffer);
        // std::cout << "literal found: " << "\n";
    }else if (*_mnemonic == "word") {
    ...
    if (_operand != nullptr) {
        // i use this check to for literals statemnets in intermediate file 
        // since they are statemnet that are generated by the assembler
        // they doesn't follow other instructions conditions e.g the mnemonic 
        // must be a directive or something that is in op_table.
        // without this check the set_operand method will raise the flag error:
        // "invalid_combination: Invalid operand type for mnemonic"
